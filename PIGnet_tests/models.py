...
class GIN(nn.Module):
	def __init__(self, in_dim, hid_dim, out_dim):
		super().__init__()
		self.conv1 = GINConv(nn.Sequential(nn.Linear(in_dim, hid_dim, bias=False),
			nn.BatchNorm1d(hid_dim), nn.ReLU()))
		self.conv2 = GINConv(nn.Sequential(nn.Linear(hid_dim, hid_dim, bias=False),
			nn.BatchNorm1d(hid_dim), nn.ReLU()))
		self.conv_inter1 = GINConv(nn.Sequential(nn.Linear(1, hid_dim, bias=False),
			nn.BatchNorm1d(hid_dim), nn.ReLU()))
		self.conv_inter2 = GINConv(nn.Sequential(nn.Linear(1, hid_dim, bias=False),
			nn.BatchNorm1d(hid_dim), nn.ReLU()))
		self.conv_inter3 = GINConv(nn.Sequential(nn.Linear(1, hid_dim, bias=False),
			nn.BatchNorm1d(hid_dim), nn.ReLU()))
		self.fc = nn.Linear(hid_dim, out_dim, bias=False)
...
class GAT(nn.Module):
	def __init__(self, in_dim, hid_dim, out_dim):
		...
		self.conv_inter1 = GATConv(1, hid_dim)
		self.conv_inter2 = GATConv(1, hid_dim)
		self.conv_inter3 = GATConv(1, hid_dim)
		self.fc = nn.Linear(hid_dim, out_dim, bias = False)
